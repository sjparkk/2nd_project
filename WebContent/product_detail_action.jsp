...
if(request.getMethod().equalsIgnoreCase("GET")){
	...
	Cart findCart = cartService.findCartByProductNoAndUserNo(new Cart(0, Integer.parseInt(productQty),
			userNo, Integer.parseInt(productNoStr)));
	
	if(findCart == null) {
		findCart =  new Cart(0, Integer.parseInt(productQty), userNo, Integer.parseInt(productNoStr));
		cartService.insertCart(findCart);
		response.sendRedirect("cart.jsp");
		return;
	} else {
		findCart = new Cart(findCart.getCartno(), findCart.getProductqty()+Integer.parseInt(productQty),
				userNo, Integer.parseInt(productNoStr));
