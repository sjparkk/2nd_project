...
<%
   ...
   /*
   List 비어져있을때 insert하기
   */
   if(productNoStr!=null && productQtyStr!=null){
       Cart updateCart = new Cart(0,Integer.parseInt(productQtyStr),userno,Integer.parseInt(productNoStr));
      if(cartList.size()==0){
         cartService.insertCart(updateCart);
         //userno수정
         cartList = cartService.findCartListByUserNo(userno);
          response.sendRedirect("cart.jsp");
          return;
      }
      /*
       for(Cart cart : cartList){
            if(cart.getProductno()==Integer.parseInt(productNoStr) && cart.getUserno()==userno)
               {
                  cartService.UpdateCartByUserNoAndProductNo(updateCart);
                  return;
               } else {
                  cartService.insertCart(updateCart);
                  return;
   ...
   /*
   //detail에서 넘어온 파라메터 카트 프로덕트넘버랑 유저넘버가 일치하는 테이블이 있을때 update를 하고 없을때는 새로운 새로운 카트를 insert하는 문을 실행한다.
               ...
               <div class="table-responsive">
                     ...
                     %>
                        ...
                        <tr>
                           <td>
                              <div class="media">
                                 <div class="d-flex">
                                    <img src="assets/img/gallery/<%=productNo.getProductImage() %>"alt="" />
                                 </div>
                           ...
                           <td>
                              <h5><%=new DecimalFormat("#,###").format(productNo.getProductPrice())%>원</h5>
                           </td>
                           <td>
                           <!-- 수량조정버튼 -->
                              ...
                              <input id="updateCartQty" class="product_count_item input-number" type="text" value="<%=cart.getProductqty() %>" min="0" max="10">                            
                           ...
                           <td>
                              ...
                              <!------------------ form -------------------->
                              <!-- 원본작업 시작 -->
                        <form name="cartForm" action="cart_update_action.jsp" method="POST">
                        ...
                        <tr class="bottom_button">
                           <td><button type ="button" class="btn_1"  onclick="updateCart()">Update Cart</button></td>
                        ...
                        <tr>
                           ...
                           <td>
                              <h5>￦<%=new DecimalFormat("#,###").format(totalPrice) %></h5>
                           </td>
                        </tr>
                     </tbody>
                  </table>
                  <div class="checkout_btn_inner float-right">
                     <a class="btn_1" href="shop.jsp">계속 쇼핑하기</a> <a
                        class="btn_1 checkout_btn_1" href="javascript:buy();">결제하기</a>
                  </div>
               </div>
            </div>
      </section>
